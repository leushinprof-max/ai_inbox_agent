...
5. Use Preview & test → View request for a conversation, a manual example, Rewrite and Missing knowledge. Verify two input messages, event timestamps (or null), exact material URLs, operator values and the raw body. Preview does not call a model. **Save version** creates an unpublished version; neither saving nor deploying changes the live release.
