...
## Provider timestamp reconciliation, 2026-09-10

The owner authorized applying `20260910081857_reconcile_provider_timestamps.sql` to the dedicated `ai-inbox-dev` database and refreshing the reported conversation. The CLI dry run selected only this migration; application succeeded and remote migration history independently confirmed its exact version. The known nonfatal pg-delta certificate-file cache warning recurred after successful application. No web or worker redeployment was needed because both already call the updated ingestion RPC.

A fresh HeyReach API snapshot contained five messages where Inbox had six. Ingestion reduced the stored history to five, preserved both existing send-operation links and AI markers, and retained inbound revision and read state. A second ingestion inserted zero messages and left one copy of the affected outgoing message. This verification used provider reads and database reconciliation only; no message was sent. Other affected histories are repaired on their next normal refresh, webhook read or import.

All 104 unit/SQL tests, lint, typecheck, build and diff checks passed before application. An offline copy of the reported conversation also matched the five-message provider history after two reads. Hosted catalog checks confirmed that only the server role can call the ingestion RPC and its private invoker helper, both with empty search paths. Security advisors were unchanged: five private-table policy notices, 32 authenticated definer warnings and the existing [leaked-password protection warning](https://supabase.com/docs/guides/auth/password-security#password-strength-and-leaked-password-protection). The release receipt and message-level evidence remain in ignored `.artifacts/provider-reconciliation-*` files.
