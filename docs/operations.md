...
Web requires the public Supabase URL/key, `SUPABASE_SECRET_KEY`, `INBOX_ENCRYPTION_KEY`, `INBOX_APP_URL` and `OPENAI_API_KEY` for Agent Test. Worker uses the same server/database/encryption/model configuration. Product admin → AI configuration → Models selects separate classification and reply models through published versions; `INBOX_MODEL` is the fallback for Server default and older versions, and defaults to `gpt-4.1-mini-2025-04-14`. Keep this fallback consistent on web and worker. Update both binaries before saving a configuration with model fields; see the [model configuration rollout](labels-and-ai-configuration.md#product-admin). `PORT` is the worker HTTP listener. Do not expose server keys through a `NEXT_PUBLIC_` variable. Keep encryption-key backups; changing that key without re-encrypting stored connections makes them unreadable.
