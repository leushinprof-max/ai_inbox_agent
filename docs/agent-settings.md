...
The editor has six steps: Background, Communication, References, Follow-ups, Test and Settings. Back/Next navigation and the step tabs change the visible step without saving or discarding edits. The header, step navigation and footer stay fixed while the form scrolls. Test keeps its message composer above the fixed Back/Next footer. Next uses a right arrow without a destination label. Finish setup on Settings saves changes and returns to the agent list only after success; validation and partial-save errors keep the editor open. Save changes in the header saves without leaving. Reply coverage always selects one of three presets: Only positive, Positive + neutral, or All replies. Stored combinations that match no preset are projected to the nearest inclusive preset in the editor and marked unsaved; stored settings change only on Save.

- Background holds About your company: the company name, description and offer in one text. Agents that store them as separate fields are merged into it when opened, and the merged text is persisted only when the agent is saved. Selling points use add/edit dialogs.
...
- References contains reply examples and materials, with add/edit dialogs and local deletion. PDF materials can be edited except for their file link; the editor adds only link materials.
- Follow-ups contains the Automatic follow-ups switch, the number of follow-ups (1–5), a wait period in days before each follow-up (1–365), writing instructions and up to three message examples. Follow-ups are off until enabled; [Leads and follow-ups](leads-and-follow-ups.md) describes scheduling.
- Settings contains the internal name, reply language, a three-segment reply-coverage selector and sender assignments. Workspace default is placed next to the Settings heading. Language and writing-form choices use keyboard-accessible menus styled to match the application. A compact writing-form selector sits in each sender row when the language is Russian, automatic or another language that may need it. English, German and Dutch hide the selector without clearing its saved value.
- The Active switch in the header is saved with the other edits; saving the editor with Active on requires a company description and a conversation goal. The switch on an agent card in the list activates or pauses the agent immediately without that check; draft agents show Finish setup instead of a switch.
...
`agent-playground-actions.ts` authenticates each request and requires workspace owner/admin access. `agent-playground-context.ts` loads workspace-scoped records with the caller's RLS client, validates resource ownership, and combines the unsaved agent snapshot with the published product configuration.

The playground runs the reply stage only, through the per-workspace model-test limit (30 per hour, `reserve_agent_test`) and the `runRecordedAI` audit path. Redraft replaces the latest generated reply only after a successful result; failures keep the previous reply visible and allow retry. Adjust instructions applies operator guidance to this test, with a separate link to edit the agent's Communication settings. Missing knowledge is answered within the test. A no-reply decision is displayed without inventing a message.

Start over, choosing another sender, and any edit to the agent or to a sender's writing form clear the synthetic transcript, operator instructions and answers. Late results from an invalidated run are discarded. Merely switching editor tabs retains the test. Demo mode can show manual test messages but does not generate simulated model responses.
