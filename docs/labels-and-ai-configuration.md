...
In v2 configurations the editable instructions are classification, every system label, the reply agent template ([Reply agent configuration v2](reply-agent.md)) and the follow-up agent template. Legacy v1 configurations also stored separate blocks for whether a reply is needed, draft writing, rewrite, missing knowledge, the agent settings template and new-agent defaults; these remain readable in saved versions. Existing tenant agents retain their saved settings when defaults change. Unknown template variables are rejected; substitution runs once and executes no code.
