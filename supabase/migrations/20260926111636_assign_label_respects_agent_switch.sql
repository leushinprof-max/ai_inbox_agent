-- A manual label is saved even when the conversation's agent is off; only the
-- reply draft request is skipped, matching automatic classification.
...
begin
  ...
  if a.id is not null and c.agent_enabled and not c.archived and not c.contact_stopped
    and exists (
      select 1 from public.workspace_labels
