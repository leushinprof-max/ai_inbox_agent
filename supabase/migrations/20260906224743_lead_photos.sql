-- Keep existing photos when older workers or incomplete provider profiles omit them.
ALTER TABLE public.conversations ADD COLUMN contact_photo_url text;
...
begin
  ...
  insert into public.conversations(workspace_id,provider_conversation_id,sender_id,sender_name,contact_name,contact_company,contact_position,contact_photo_url)
    values(p_workspace,p_data->>'id',(p_data->>'senderId')::bigint,p_data->>'senderName',p_data->>'contactName',coalesce(p_data->>'company',''),coalesce(p_data->>'position',''),nullif(p_data->>'photoUrl',''))
    on conflict(workspace_id,sender_id,provider_conversation_id) do update set sender_name=excluded.sender_name,contact_name=excluded.contact_name,contact_company=excluded.contact_company,contact_position=excluded.contact_position,contact_photo_url=coalesce(excluded.contact_photo_url,public.conversations.contact_photo_url) returning * into c;
