-- First reply uses the earliest incoming provider timestamp across the entire stored history.
-- The partial index also supports histories imported or corrected out of order.
create index if not exists messages_first_reply_idx
  on public.messages(workspace_id,conversation_id,occurred_at) where direction='inbound';
    ...
    and not exists (
      ...
      where (case f->>'field'
        ...
        when 'first_reply' then (select case
          ...
          when f->'values'->>0 in ('this_week','this_month') then
            first_at >= (date_trunc(case when f->'values'->>0='this_week' then 'week' else 'month' end,
